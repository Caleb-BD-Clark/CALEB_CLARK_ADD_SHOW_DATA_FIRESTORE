  ...
  db.collection("teams")
    ...
    .then((snapshot) => {
      snapshot.forEach((doc) => {
        ...
        const updatedScorers = doc.data().topScorers
          .filter((s) => s !== "Hazard")
          .concat("Crispo");

        db.collection("teams").doc(doc.id).update({
          topScorers: updatedScorers,
        });
      });
    });
...
function queryC() {
  // Update Real Madrid FC away jersey to Purple
  db.collection("teams")
    .where("teamName", "==", "Real Madrid FC")
    .get()
    .then((snapshot) => {
  ...
  // Update FC Barcelona away jersey to Pink
  db.collection("teams")
    .where("teamName", "==", "FC Barcelona")
    .get()
    .then((snapshot) => {
